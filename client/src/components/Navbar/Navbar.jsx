...
const Navbar = () => {
  ...
  const handleLogout = async () => {
    try {
      ...
      const message = err?.response?.data || "Logout failed. Please try again.";
      toast.error(message);
    }
  };
    ...
    <div className={active || pathname !== "/" ? "navbar active" : "navbar "}>
      <div className="container">
        ...
        <div className="links">
          ...
          <Link to="/login" className="link" key={333}>
            <span>Sign in</span>
          </Link>

          {!current_user?.isSeller && (
            <span onClick={(e) => navigate("/becomeSeller")}>
              Become a Seller
            </span>
          )}
          {!current_user && (
            ...
            <div className="user" onClick={() => setopen(!open)}>
              ...
              {open && (
                <div className="options">
                  {current_user.isSeller && (
                    <>
                      <Link className="link" key={555} to="/mygigs">
                        Gigs
                      </Link>
                      <Link className="link" key={999} to="/add">
                        Add New Gig
                      </Link>
                    </>
                  )}
